import { createSbServer } from '@/lib/supabase-server'
import NewNote from './new-note'

export default async function EmployeeDetail({ params }: { params: { id: string } }) {
  const sb = createSbServer()
  const { data: employee } = await sb.from('employees').select('*').eq('id', params.id).single()
  const { data: notes } = await sb.from('progress_notes').select('*').eq('employee_id', params.id).order('note_date', { ascending: false })

  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-semibold text-blue-800">{employee?.name}</h2>
          <p className="text-sm text-slate-600">{employee?.department} • {employee?.employment_type}</p>
        </div>
        <a
          className="btn"
          href={`/notes/new?employee=${params.id}`}  // NEW: deep link to form with ID
        >
          New Note
        </a>
      </div>

      {/* Inline form (kept) */}
      <NewNote employeeId={params.id} />

      {/* Notes list */}
      <div className="grid gap-4">
        {notes?.map((n) => (
          <div key={n.id} className="card">
            <div className="text-sm text-slate-600">
              {new Date(n.note_date).toLocaleDateString()} • {n.meeting_location}
            </div>
            <div className="flex flex-wrap gap-2 my-2">
              {(n.areas_of_need as string[] ?? []).map((a) => (
                <span key={a} className="badge">{a}</span>
              ))}
            </div>
            <p className="text-sm whitespace-pre-wrap">{n.meeting_summary}</p>
            {n.file_url && (
              <a className="text-blue-700 underline text-sm" href={n.file_url} target="_blank">
                View file
              </a>
            )}
          </div>
        ))}
      </div>
    </div>
  )
}
