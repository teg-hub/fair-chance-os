  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card className="p-4 col-span-2 lg:col-span-1">
        ...
        <div className="h-36">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={(em||[]).map(x=>({ x: x.month?.slice(0,10), y:x.engagements }))}>
      ...
      <Card className="p-4">
        ...
        <div className="h-36">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={(byDept||[]).map(x=>({ x:x.department||'Unassigned', y:x.engagements }))}>
              <XAxis dataKey="x" hide/>
              <YAxis hide/>
              <Tooltip/>
              <Bar dataKey="y" />
      ...
      <Card className="p-4">
        ...
        <div className="h-36">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={(byAon||[]).map(x=>({ x:x.area_of_need, y:x.engagements }))}>
              <XAxis dataKey="x" hide/>
              <YAxis hide/>
              <Tooltip/>
              <Bar dataKey="y" />
